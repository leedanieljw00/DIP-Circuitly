...
const syncState = async () => {
    if (State.studentId) {
        await window.ProfileService.updateProgress(State.studentId, {
            xp: State.xp,
            hearts: State.hearts,
    ...
    if (State.view !== ROUTES.LOGIN && State.view !== ROUTES.PROFILES) {
        ...
        setTimeout(() => {
            ...
            if (!State.isAdmin) {
                ...
                if (heartBtn) heartBtn.onclick = async () => {
                    if (confirm('Refill hearts to MAX?')) {
                        State.hearts = 5;
                        State.nextHeartRestoreTime = null;
                        if (window.ProfileService) {
                            await window.ProfileService.updateProgress(State.studentId, { hearts: 5 });
                        }
                        render();
                    }
                };
            ...
            if (window.Quiz) {
                component = window.Quiz({
                    ...
                    onComplete: async (result) => {
                        ...
                        await window.ProfileService.updateStats(State.studentId, State.activeTopicId, earnedScore, timeSpent);
                        ...
                        await syncState(); // Save basic state
