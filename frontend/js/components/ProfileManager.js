window.ProfileManager = function ({ onBack, onProfileSwitched }) {
    ...
    addBtn.onclick = async () => {
        ...
        const result = await window.ProfileService.addProfile({
            name: name,
            studentId: id,
            classGroup: group,
            username: id, // Default username to ID if not provided
            password: 'password123' // Default password for management-added profiles
        });
        ...
        profiles.forEach(p => {
            ...
            resetBtn.onclick = async () => {
                if (confirm(`Reset ALL progress for ${p.name}? This cannot be undone.`)) {
                    await window.ProfileService.resetProfile(p.studentId);
            ...
            delBtn.onclick = async () => {
                if (confirm(`Delete profile for ${p.name}?`)) {
                    await window.ProfileService.deleteProfile(p.studentId);
                    renderList();
                }
            };
