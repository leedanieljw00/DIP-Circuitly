...
function render() {
    ...
    switch (State.view) {
            ...
            if (window.Home) {
                component = window.Home({
                    ...
                    onStart: (topicId) => {
                        ...
                        // Intercept Test Topic (ID 9)
                        if (Number(topicId) === 9) {
                            State.view = ROUTES.SUB_TOPICS;
                            render();
                            return;
                        }
        ...
        case ROUTES.SUB_TOPICS:
            if (window.SubTopicSelection) {
                component = window.SubTopicSelection({
                    onSelect: (subId) => {
                        // Composite ID: "9_kirchhoff", etc.
                        State.activeTopicId = `9_${subId}`;
                        State.view = ROUTES.QUIZ;
                        render();
                    },
                    onBack: () => {
