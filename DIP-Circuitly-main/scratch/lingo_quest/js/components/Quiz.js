window.Quiz = function ({ topicId, currentSkill = 10, onComplete, onExit }) {
    console.log(`Quiz Initialized for Topic ${topicId}. Current Skill: ${currentSkill}`);
    const questions = window.DataService.getAdaptiveQuestions(topicId, currentSkill);
    let currentIndex = 0;
    let score = 0;
    let skill = currentSkill; // Local tracking of skill
    ...
    const adaptiveStats = {
        ...
        lastDifficultyCheck: currentSkill > 60 ? 3 : (currentSkill > 30 ? 2 : 1)
    };
    ...
    checkBtn.onclick = () => {
        if (!isAnswered) {
            ...
            if (selectedOption === q.correctAnswer) {
                ...
                // Skill Update (Elo-lite)
                // If correct, increase skill based on difficulty
                const diff = q.difficulty || 1;
                const gain = Math.round(5 * diff);
                skill += gain;
                if (skill > 100) skill = 100;

                console.log(`Correct! Difficulty: ${diff}, Gain: ${gain}, New Skill: ${skill}`);

                feedbackOverlay.innerHTML = `<h2 style="margin-bottom:12px">Nicely Done! (+${gain} Skill)</h2>`;
                ...
                // Skill Update (Elo-lite)
                // If incorrect, decrease skill based on difficulty
                const diff = q.difficulty || 1;
                const loss = Math.round(5 / diff); // Lose less if hard, lose more if easy
                skill -= loss;
                if (skill < 0) skill = 0;

                let explanationHTML = `<h2 style="margin-bottom:12px">Incorrect (-${loss} Skill)</h2>
    ...
    function nextQuestion() {
        ...
        // Check if skill has drifted enough to warrant a different difficulty
        let currentDiff = 1;
        if (skill > 60) currentDiff = 3;
        else if (skill > 30) currentDiff = 2;

        if (currentDiff !== adaptiveStats.lastDifficultyCheck) {
            console.log(`Adaptive: Skill crossed threshold. Switching target difficulty from ${adaptiveStats.lastDifficultyCheck} to ${currentDiff}.`);
            adaptiveStats.lastDifficultyCheck = currentDiff;
        }

        // Swap upcoming questions (if any left)
        // We only swap the NEXT few to avoid jarring changes or to ensure flow.
        // For MVP: Swap the very next question if it doesn't match target difficulty.

        const nextIdx = currentIndex + 1;
        if (nextIdx < questions.length) {
            const nextQ = questions[nextIdx];
            // Check if next question matches our new target difficulty
            // (Allow fuzzy match? No, let's be strict for demonstration)
            // Note: generated questions (topic 8) might not have 'difficulty' set correctly in all paths, ensure default.
            const nextQDiff = nextQ.difficulty || 1;

            if (nextQDiff !== currentDiff) {
                console.log("Adaptive: Swapping next question to match new difficulty.");
                const excludeIds = questions.map(q => q.id);
                const newQ = window.DataService.getNextQuestion(topicId, currentDiff, excludeIds);

                if (newQ) {
                    questions[nextIdx] = newQ;
                    console.log("Adaptive: Swap success.");
                }
            }
        }
        currentIndex++;
